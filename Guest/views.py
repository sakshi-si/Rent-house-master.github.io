...
from user.models import Contact, User, Room , House
...
def search(request):
    ...
    if request.method == 'GET':
        ...
        context.update({'q':q})
        results={}
        if typ == 'House' and (bool(House.objects.filter(location=q)) or bool(House.objects.filter(city=q)) or bool(House.objects.filter(cost=q))):
            results = House.objects.filter(location=q)
            results = results | House.objects.filter(city=q)
            results = results | results | House.objects.filter(cost=q)
            ...
            # results = results | results | Room.objects.filter(cost=q)
        elif typ == 'Apartment'  and (q.isnumeric()):
            results = Room.objects.filter(location=q)
            results = results | Room.objects.filter(city=q)
            results = results | results | Room.objects.filter(cost=q)
        else:
...
def agreement(request):
    template = loader.get_template('agreement.html')
    name = 'Owner'
    context = {}
    id = request.GET['id']
    room = Room.objects.get(room_id=id)
    ...
    # print(amount)
    context.update({"value":room,"date":today,"owner":name,"amount":amount,"renter":user})
    # print(renter)
    print(context)
    # print(context)
